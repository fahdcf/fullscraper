...
async function startUnifiedScraper({
  ...
  try {
    ...
    // Convert format to lowercase for existing scrapers
    const scraperFormat = format.toLowerCase();
  ...
  const batchSize = 15;
    ...
    // Export results using the existing result processor
    ...
    // Construct the full file path from the exported filename
    const finalFilePath = path.join(__dirname, '../results', exportedPath);
